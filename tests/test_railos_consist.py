...
def test_consist_access(
    consist_library: railosc_library.ConsistLibrary,
    random_entry: typing.Tuple[str, typing.Dict[str, int]],
) -> None:
    ...
    assert (
        consist_library.max_speeds("gb")[random_entry[0]]
        == random_entry[1]["max_speed"]
    )
...
def test_header_creation(
    ...
    assert consist_library.headers("gb")[
        random_entry[0]
    ] == "{reference};{description};{start_speed};" + ";".join(
        str(i) for i in random_entry[1].values()
    )
